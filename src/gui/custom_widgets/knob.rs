// Vendored from https://github.com/ardura/Actuate/blob/main/src/CustomWidgets/ui_knob.rs
// Fixed some LSP complaints, some light refactoring, nothing changed apart from that
//
...
use nih_plug_egui::egui::{
  self, Align2, Color32, CornerRadius, FontId, Modifiers, Pos2, Rect, Response,
  Rgba, Sense, Shape, Stroke, Ui, Vec2, Widget,
...
static DRAG_AMOUNT_MEMORY_ID: LazyLock<egui::Id> =
  LazyLock::new(|| egui::Id::new((file!(), 1)));
...
impl<'a, P: Param> SliderRegion<'a, P> {
  ...
  fn drag(&self, ui: &Ui, drag_delta: Vec2) {
    let drag_multiplier = ui.input(|mem| match mem.modifiers {
      // And ctrl dragging should switch to a more granular input method
      Modifiers { ctrl: true, .. } => SEMI_GRANULAR_DRAG_MULTIPLIER,
      // And shift dragging should switch to a VERY granular input method
      Modifiers { shift: true, .. } => GRANULAR_DRAG_MULTIPLIER,
      _ => NORMAL_DRAG_MULTIPLIER,
    });

    // Remember the initial position when we started with the granular drag. This value gets
    ...
    let start_value = if Self::get_drag_amount_memory(ui) == 0.0 {
      ...
      Self::set_drag_normalized_start_value_memory(ui, normalized_value);
      normalized_value
    } else {
    ...
    let new_value = start_value + (total_drag_distance * drag_multiplier);
    self.set_normalized_value(new_value.clamp(0.0, 1.0));
  }
  ...
  fn handle_response(&self, ui: &Ui, response: &mut Response) -> f32 {
    ...
    if let Some(_clicked_pos) = response.interact_pointer_pos() {
      self.drag(ui, response.drag_delta());
  ...
  fn ui(mut self, ui: &mut Ui) -> Response {
    ...
    ui.vertical(|ui| {
      ...
      let center = match self.layout {
        KnobLayout::Default | KnobLayout::SquareNoLabel => {
      ...
      if self.show_label {
        ...
        // Setting text color
        let text_color = if self.text_color_override == Color32::PLACEHOLDER {
          self.line_color
        } else {
        ...
        } else if self.layout == KnobLayout::HorizontalInline {
          painter.text(
            label_pos,
            Align2::CENTER_CENTER,
