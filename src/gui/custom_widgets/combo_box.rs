// Vendored from https://github.com/ardura/Actuate/blob/main/src/CustomWidgets/ComboBoxParam.rs
...
pub struct ParamComboBox<'a, P: Param> {
  ...
  options: &'a Vec<String>, // Options for the ComboBox
}
...
impl<'a, P: Param> ParamComboBox<'a, P> {
  ...
  fn set_selected_value(&self, selected_value: String) {
    ...
    if let Some(normalized_value) =
      self.param.string_to_normalized_value(&selected_value)
    {
...
impl<'a, P: Param> Widget for ParamComboBox<'a, P> {
  fn ui(self, ui: &mut Ui) -> Response {
    ...
    let response =
      ComboBox::from_id_salt(self.param.name().to_owned() + &self.id_name)
